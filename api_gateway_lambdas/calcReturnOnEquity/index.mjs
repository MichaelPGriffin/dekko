...
export const handler = async(event) => {
    ...
    const { metricValue: net_income, filing_date } = await netIncome(symbol, periodOffset);
    const { metricValue: shareholders_equity } = await equity(symbol, periodOffset);
...
const netIncome = async (symbol, periodOffset) => await FindMetric(symbol, ['net_income_loss'], periodOffset);
